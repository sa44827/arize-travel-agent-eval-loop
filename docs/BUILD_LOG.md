...
### #44 — Tone judge negative-class validated (4/4, 100%) + real quota confirmed
**What:** external review correctly flagged that golden.csv had zero unprofessional-tone examples -- a judge that always said "professional" would score 100% on it, so the tone judge's actual sensitivity was never tested. User raised their Gemini key's rate limit; verified with 6/6 sequential successful calls (not a single lucky ping like #39's false positive) that quota is genuinely available now.
**Fix:** added evals/golden/tone_negative_examples.csv (4 synthetic, deliberately unprofessional replies: dismissive/rude, overly casual, exposes internal systems, sends user to a competitor) and scripts/validate_tone_negatives.py to test the tone judge against them directly (not tied to real Phoenix spans, since the fixed agent doesn't currently produce bad-tone output to sample from -- stated honestly as synthetic, not claimed as real traffic).
**Result: 4/4 caught, 100%.** The judge correctly labeled all four unprofessional with accurate explanations (named the rudeness, the internal-systems leak, and the competitor referral specifically). This converts "tone judge sensitivity is untested" from a named gap into a validated claim.

### #45 — Re-validation with real quota surfaced a NEW self-inflicted regression, found and fixed same session
**What:** with real Gemini quota confirmed (6/6 sequential calls), re-ran scripts/validate_judge.py to finally close out #39. Groundedness came back clean: 89% agreement, kappa 0.79 -- matches the original round-2 number exactly, genuinely re-verified live (not the pre-fix number cited before).
**Tone collapsed to 26% agreement, kappa 0.00 (14 of 19 flipped to "unprofessional").** Root cause: the earlier TODAY_DATE fix (#39-era) added a tone criterion -- "unprofessional... is factually confused about dates given today's date" -- meant to stop the judge misreading past-tense weather replies as hallucinations. Real wall-clock time has since moved forward (golden set built ~2026-09-20/21, re-run on 2026-09-23), so travel dates that were "future" when the set was built are now genuinely in the past -- and the judge, applying that rule exactly as written, flagged nearly every reply as "factually confused" for treating fixture bookings as available. Same class of bug as #31 (judge lacking temporal context) but inverted: this time the date-awareness fix itself became the source of false positives, and it will keep recurring on every future re-run as real time advances past more of the fixture's 2026 dates.
**Fix:** removed the date-confusion clause and the TODAY_DATE line from the TONE template entirely. Tone judges HOW something is said, not whether temporal reasoning about fictional demo data is correct -- that reasoning belongs to groundedness (where TODAY_DATE stays) or nowhere, not to tone. Re-ran: tone back to 19/19 = 100%, kappa 1.00.
**Also added (external review's suggestion, independently valid):** evals/golden/tone_negative_examples.csv, 4 synthetic deliberately-unprofessional replies (rude, overly casual, internal-systems leak, sends to a competitor) plus scripts/validate_tone_negatives.py, since the real golden set has zero unprofessional examples -- a judge that always said "professional" would score 100% on it. Result: 4/4 caught, 100%. Tone judge is now validated on both classes, not just the positive one.
**Lesson:** a fix for one date-related false positive (#31/#39) was over-applied to a different judge (tone) where it didn't belong, and the interaction with real elapsed time made it fragile in a way that wasn't visible until quota allowed a genuine re-run days later. Scope every fix to the judge it's actually needed for -- "add today's date" is not a universally safe patch.
